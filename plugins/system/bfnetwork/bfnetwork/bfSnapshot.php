...
final class bfSnapshot
{
	...
	/**
	 * Fix Joomla 3.5.1 Brain Farts on windows/linux hosts with capitalizations
	 * @see https://github.com/joomla/joomla-cms/pull/9902
	 * @see https://github.com/joomla/joomla-cms/issues/9893
	 */
	private function fixJoomlaCrap()
	{
		$a = JPATH_BASE . '/media/mod_languages/images/si_LK.gif';
		$b = JPATH_BASE . '/media/mod_languages/images/si_lk.gif';
		$c = JPATH_BASE . '/media/mod_languages/images/si_LK.gif.backup';
		if (file_exists($a))
		{
			@copy($a, $c);
			@unlink($a);
	...
	private function initDb()
	{
		...
		if (!$dbVerString && get_class($this->db) == 'JDatabaseDriverMysql')
		{
			$dbVerString = @mysql_get_server_info($this->db->getConnection());
		}


		if (!$dbVerString && method_exists($this->db, 'getConnection'))
		{
